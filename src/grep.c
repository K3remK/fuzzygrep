...
{
  {"approx",           optional_argument, nullptr, 'X'},
...
static struct
{
  ...
  { "fuzzy", 0, FuzzyCompile, FuzzyExecute  },
#if HAVE_LIBPCRE
...
main (int argc, char **argv)
{
  ...
  while (prev_optind = optind,
    ...
    switch (opt)
      ...
      case 'X':
        matcher = setmatcher ("fuzzy", matcher);
        if (optarg)
          approx_threshold = atoi(optarg);
        else
          approx_threshold = 1;  // default distance if not specified
        //printf("Using approximate matching with threshold = %d\n", approx_threshold);  
        break;
